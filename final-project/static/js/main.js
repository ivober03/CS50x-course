    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function validatePasswords() {
        ...
        if (passwordValue.length < 6) {
          passwordField.classList.add('is-invalid');
          confirmationError2.textContent = 'Password is too short!';
          submitBtn.disabled = true; // disable submit button
        } else {
          passwordField.classList.remove('is-invalid');
          confirmationError2.textContent = '';
          submitBtn.disabled = false; // enable submit button
        }
      ...
      // to prevent form submission if the passwords do not match
      signupForm.addEventListener('submit', function(event) {
        const passwordValue = passwordField.value;
        const confirmationValue = confirmationField.value;

        if (passwordValue !== confirmationValue || passwordValue.length < 6) {
